...
def _extract_name(url, service_type=None, project_id=None):
    ...
    url_parts = list(filter(lambda x: x != project_id, url_path.split('/')))
...
class Proxy(adapter.Adapter):
    ...
    def _report_stats_statsd(self, response):
        ...
        key = '.'.join(
            ...
            + name_parts)
        self._statsd_client.timing(key, int(response.elapsed.microseconds / 1000))
