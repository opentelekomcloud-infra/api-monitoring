...
also may be used for any other activities (i.e. provision VM and deploy
software there). The flow of monitored data is organized in a pipeline from
collecting metric data on individual client instances, aggregating them on the
clients, storing and processing data, and finally visualizing and alerting
...
as metrics to the `Telegraf` instance in any supported metric format.
`Telegraf` is then responsible for sending data to a `InfluxDB` instance, to
store it as a time series data. This data is unique for each environment where
API monitoring is working, meaning that its metrics are the same, but the
values are different for different environments (i.e. duration of server
